...
def login_view(request):
    ...
    try:
        ...
        return error('No patient profile found for this user.', 404)
...
def doctor_login_view(request):
    ...
    try:
        ...
        return error('No doctor profile found for this user.', 404)
...
def doctor_patient_detail_view(request, patient_id):
    ...
    moca_data = [
        {
            'id': m.id,
            'total_score': m.total_moca_score,
...
def debug_plans_view(request):
    ...
    all_patients = Patient.objects.all().select_related('assigned_doctor')
    patients_data = [{
        'id': p.id,
        'name': p.name,
        'email': p.email,
    ...
    return success({
        ...
        'total_plans_in_db': len(plans_data),
...
def clinical_plans_view(request):
    ...
    if request.method == "GET":
        if role == 'doctor':
            # Doctor sees all plans they've assigned, can filter by patient
            ...
            types = ['exercise', 'diet', 'task', 'prescription']
            result = {}
...
def notifications_list_view(request):
    """Return all notifications for the logged-in patient."""
    patient_id = request.session.get('patient_id')
    if not patient_id:
        return error('Not authenticated.', 401)
    ...
    data = [
        {
            ...
            'plan_type': n.plan.plan_type,
            'doctor_name': n.plan.doctor.name,
            'special_instructions': n.plan.special_instructions
        }
...
def mark_all_notifications_read_view(request):
    """Mark all notifications for the patient as read."""
    patient_id = request.session.get('patient_id')
    if not patient_id:
        return error('Not authenticated.', 401)
...
def unread_count_view(request):
    """Return the unread notification count for the patient."""
    patient_id = request.session.get('patient_id')
    if not patient_id:
        return error('Not authenticated.', 401)
...
def google_login_view(request):
    try:
        ...
        is_complete = all([patient.age, patient.dob, patient.phone])
...
def complete_patient_profile_view(request):
    """Update missing patient profile information."""
    patient_id = request.session.get('patient_id')
    if not patient_id:
...
def complete_doctor_profile_view(request):
    """Update missing doctor profile information."""
    doctor_id = request.session.get('doctor_id')
    if not doctor_id:
        return error('Not authenticated as doctor.', 401)
