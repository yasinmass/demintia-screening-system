        ...
        <DashboardLayout role="doctor" title="Clinical Registry">
            <div className="page-header mb-8 border-b pb-4 flex justify-between items-end">
                ...
                <button
                    ...
                    onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = showEnroll ? '0 8px 28px rgba(239,68,68,0.5)' : '0 8px 28px rgba(37,99,235,0.5)'; }}
                    onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = showEnroll ? '0 4px 20px rgba(239,68,68,0.4)' : '0 4px 20px rgba(37,99,235,0.4)'; }}
                >
                    <span style={{ fontSize: '16px' }}>{showEnroll ? '✕' : '＋'}</span>
            ...
            <div className="flex flex-col md:flex-row gap-4 mb-8">
                ...
                <div className="flex gap-2">
                    {[
                        { key: 'all', label: 'All', activeColor: '#374151', activeBg: '#f3f4f6', dot: null },
                        { key: 'High', label: 'High Risk', activeColor: '#dc2626', activeBg: '#fef2f2', dot: '#dc2626' },
                        { key: 'Moderate', label: 'Moderate', activeColor: '#b45309', activeBg: '#fffbeb', dot: '#f59e0b' },
                        { key: 'Low', label: 'Low Risk', activeColor: '#059669', activeBg: '#ecfdf5', dot: '#10b981' },
                    ].map(f => (
                        <button
                            key={f.key}
                            ...
                            style={{
                                padding: '7px 16px',
                                borderRadius: '8px',
                                border: filter === f.key ? `1.5px solid ${f.activeColor}40` : '1.5px solid #e5e7eb',
                                background: filter === f.key ? f.activeBg : '#fff',
                                color: filter === f.key ? f.activeColor : '#9ca3af',
                                fontSize: '11px',
                                fontWeight: '700',
                                cursor: 'pointer',
                        ...
                        >
                            {f.dot && <span style={{ width: 7, height: 7, borderRadius: '50%', background: f.dot, display: 'inline-block', flexShrink: 0 }} />}
                            {f.label}
                        </button>
                    ))}
                </div>
