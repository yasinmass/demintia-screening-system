...
function Attention2Section({ onScore }) {
    ...
    const isCurrentA = currentLetter === 'A';

    return (
        <div className="card" style={{ marginBottom: 24 }}>
            <div className="card-body">
                <SectionHeader num={5} title="Attention — Tap on Letter A" marks={3} />
                <p style={{ fontSize: 14, color: 'var(--gray-500)', marginBottom: 4 }}>Letters will play every 1.5 seconds. <strong>Click the button each time you hear "A"</strong>.</p>
                <div style={{ fontSize: 12, color: 'var(--gray-400)', marginBottom: 16, padding: '8px 12px', background: '#fef3c7', borderRadius: 8, border: '1px solid #fde68a' }}>
                    ⚠️ <strong>Important:</strong> Only tap when you hear "A". Tapping on other letters (B, C, F, etc.) counts as a wrong tap and <strong>reduces your score</strong>.
                </div>
                {!running && !done && (
                    <button className="btn btn-primary btn-lg" style={{ width: '100%' }} onClick={start}>▶ Start Letter Sequence</button>
                )}
                {running && (
                    <div style={{ textAlign: 'center' }}>
                        <div style={{
                            fontSize: 96, fontWeight: 900, fontFamily: 'monospace',
                            color: isCurrentA ? '#059669' : '#9ca3af',
                            marginBottom: 8,
                            transition: 'color 0.2s',
                            textShadow: isCurrentA ? '0 0 20px rgba(5,150,105,0.3)' : 'none'
                        }}>{currentLetter}</div>
                        <p style={{ fontSize: 12, color: isCurrentA ? '#059669' : '#9ca3af', fontWeight: 700, marginBottom: 16 }}>
                            {isCurrentA ? '🟢 TAP NOW! This is "A"' : '🔴 Do NOT tap — not "A"'}
                        </p>
                        <button
                            className="btn btn-primary btn-lg"
                            style={{ width: '100%', padding: 32, fontSize: 20 }}
                            onClick={handleTap}
                        >
                            TAP — I heard "A"
                        </button>
                        <div style={{ marginTop: 12, display: 'flex', justifyContent: 'center', gap: 24, fontSize: 13, color: 'var(--gray-400)' }}>
                            <span>{idxRef.current}/{LETTER_SEQ.length} letters played</span>
                            <span style={{ color: '#059669' }}>✓ Correct: {taps.filter(t => t.correct).length}</span>
                            <span style={{ color: '#dc2626' }}>✗ Wrong: {taps.filter(t => !t.correct).length}</span>
                        </div>
                    </div>
                )}
                {done && stats && (
                    <div style={{ borderRadius: 12, overflow: 'hidden' }}>
                        <div style={{ background: finalScore >= 2 ? 'var(--success-light)' : '#fef2f2', padding: '16px 20px', borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
                            <p style={{ fontWeight: 800, color: finalScore >= 2 ? 'var(--success)' : '#dc2626', fontSize: 16, marginBottom: 4 }}>
                                ✓ Sequence Complete!
                            </p>
                        </div>
                        <div style={{ background: '#f9fafb', padding: '16px 20px' }}>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
                                <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, padding: '10px 14px' }}>
                                    <p style={{ fontSize: 11, color: '#6b7280', fontWeight: 700, textTransform: 'uppercase', marginBottom: 2 }}>Correct Taps (A)</p>
                                    <p style={{ fontSize: 20, fontWeight: 900, color: '#059669' }}>{stats.correctTaps} / {stats.totalAs}</p>
                                </div>
                                <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, padding: '10px 14px' }}>
                                    <p style={{ fontSize: 11, color: '#6b7280', fontWeight: 700, textTransform: 'uppercase', marginBottom: 2 }}>Wrong Taps (−marks)</p>
                                    <p style={{ fontSize: 20, fontWeight: 900, color: stats.wrongTaps > 0 ? '#dc2626' : '#059669' }}>{stats.wrongTaps}</p>
                                </div>
                                <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, padding: '10px 14px' }}>
                                    <p style={{ fontSize: 11, color: '#6b7280', fontWeight: 700, textTransform: 'uppercase', marginBottom: 2 }}>Missed A's (−marks)</p>
                                    <p style={{ fontSize: 20, fontWeight: 900, color: stats.missed > 0 ? '#dc2626' : '#059669' }}>{stats.missed}</p>
                                </div>
                                <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, padding: '10px 14px' }}>
                                    <p style={{ fontSize: 11, color: '#6b7280', fontWeight: 700, textTransform: 'uppercase', marginBottom: 2 }}>Total Errors</p>
                                    <p style={{ fontSize: 20, fontWeight: 900, color: stats.errors > 0 ? '#f59e0b' : '#059669' }}>{stats.errors}</p>
                                </div>
                            </div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, padding: '12px 16px' }}>
                                <span style={{ fontWeight: 700, fontSize: 13 }}>
                                    Score (0 errors=3pts, 1 error=2pts, 2-3 errors=1pt, 4+=0pt)
                                </span>
...
function LanguageSection({ onScore }) {
    ...
    function startRecognition() {
        ...
        const r = new SR();
        r.continuous = true;
        r.interimResults = true; // KEY FIX: capture words as spoken, not just at phrase end
        r.lang = 'en-US';
        ...
        r.onresult = e => {
            ...
            if (newFinal.trim()) {
                ...
                setWords(processWords(finalTranscriptRef.current));
            }
        };

        r.onerror = (err) => {
            if (err.error !== 'no-speech') console.warn('SR error:', err.error);
        };

        // Auto-restart if browser stops recognition during long pauses
        r.onend = () => {
...
function OrientationSection({ onScore }) {
    ...
    const dayOfWeekNum = String(now.getDay() === 0 ? 7 : now.getDay()); // e.g. Wednesday = 3
    ...
    const fields = [
        {
            ...
            // Accept: "4", "04", "4/3/26", "4/3/2026", etc. — just needs the correct date number
            check: (a) => a === dateNum || a.startsWith(dateNum + '/') || a.startsWith(dateNum + '-') || a.includes('/' + dateNum + '/') || a.includes('-' + dateNum + '-')
        },
        {
            label: 'Day of Week',
            // Accept: "wednesday", "wed", "3" (week number Mon=1)
            check: (a) => a === dayName || a.startsWith(dayName.slice(0, 3)) || a === dayOfWeekNum
        },
        {
            label: 'Month',
            // Accept: "march", "mar", "3", "03"
        ...
        {
            label: 'City / Location',
            check: (a) => a.length > 0 // any non-empty answer
        },
    ];
                ...
                {fields.map((f, i) => (
                    <div key={i} className="form-group">
                        ...
                        <input
                            ...
                            disabled={submitted}
                            style={submitted ? {
                                borderColor: fieldResults[i] ? 'var(--success)' : 'var(--danger)',
                                background: fieldResults[i] ? 'var(--success-light)' : '#fff0f0'
                            } : {}}
                            onChange={e => { const a = [...answers]; a[i] = e.target.value; setAnswers(a); }}
                        />
                        {submitted && (
                            <small style={{ color: fieldResults[i] ? 'var(--success)' : 'var(--danger)', fontWeight: 600, display: 'block', marginTop: 2 }}>
                                {fieldResults[i] ? '✓ Correct' : '✗ Incorrect'}
                            </small>
                        )}
                    </div>
                ))}
